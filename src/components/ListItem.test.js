...
describe('ListItem component', () => {
  ...
  it('correct Link to', () => {
    expect(component.find('Link').prop('to')).toEqual(`/artist/${props.track.artist.name}`)
  });
  it('correct Link', () => {
    expect(component.find('Link')).toHaveLength(1);
  });
  it('Link contains correct value', () => {
    expect(component.contains(<Link to={`/artist/${props.track.artist.name}`}>{props.track.artist.name}</Link>)).toBeTruthy();
  });
  it('correct Image src', () => {
    expect(component.find('Image').prop('path')).toEqual(props.track.image[3]['#text']);
  });
